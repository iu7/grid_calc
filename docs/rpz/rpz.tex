...
Поведение подсистемы описывается его активной и пассивной частью.
...
\begin{figure}
  \centering
  \includegraphics[width=\linewidth]{diagrams/common/component}
...
\begin{lstlisting}[float={},language=bash,caption={Пример корректного файла со списком черт задачи},label=lst:task-traits]
root_folder_file.txt
directory/subdirectory_file.extension

\end{lstlisting}

\begin{lstlisting}[float={},language=bash,caption={Пример корректного файла со списком выходных файлов},label=lst:task-output]
root_folder_file.txt
directory/subdirectory_file.extension
