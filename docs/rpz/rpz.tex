  ...
  \begin{tabu}{|c|c|X[c]|X[c]|}
    \hline
        ...
        Subtask     &   AgentUsed   & UID?    & Использованный узел         \\ \cline{2-4}
         ...
         Task       &    Archive    & string  & Имя архива с файлами задачи \\ \cline{2-4}
                    &    MaxTime    & int     & Максимальное время расчёта задачи, в секундах \\\cline{2-4}
                    &  StartScript  & string  & Имя скрипта, запускающего задачу \\\cline{2-4}
...
\begin{table}[h]
    ...
    \begin{tabu}{|c|c|c|X[c]|}
    	\hline
    	    ...
    	    /services/type     & POST  & port, state & \{"status":"success", "address":"ipaddr:port"\}  \\ \cline{2-4}
    	                       &  GET  &             & \{address:\{state:"...", lastbeat:"..."\}, ...\} \\ \hline
    	/services/type/address &  PUT  &    state    & {"status"="success"}                             \\ \cline{2-4}
...
\begin{table}[h]
  ...
  \begin{tabu}{|c|c|c|X[c]|}
  	\hline
  	 ...
  	 /type   &  GET   &                   & массив со списком идентификаторов объектов types \\ \cline{2-4}
  	         &  POST  &    объект type    &                 \{"status"="ok"\}                  \\ \hline
  	/type/id &  GET   &                   &                   объект type                    \\ \cline{2-4}
  	         &  PUT   &    объект type    &                 \{"status"="ok"\}                  \\ \cline{2-4}
